...
| 5.4b | The licence of every piece of other people's work the programs are made of, generated from what they are built from and checked by a test and by the interface build `[x]`, shown under **Licences** in the app `[x]`, carried in the bundle and the tools download `[ ]`. The speech library as published carries espeak-ng, which is GPL 3, so it is built without it `[ ]` | `[~]` |
