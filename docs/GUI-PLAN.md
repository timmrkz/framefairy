...
| 5.2 | `Frame Fairy.app` built by `make app` and started by `make run`, so what Tim runs every day is what a customer runs: the same `Info.plist`, the same privacy prompts, the same tools inside the same folder. Wails' own Taskfile layout is not adopted, see [PACKAGING.md](PACKAGING.md#how-the-building-works) | `[~]` |
