...
| 5.4 | The first run: the speech model fetched by the app itself `[x]`, the one question asked once and remembered `[x]`, an Anthropic API key stored in the keychain `[x]`, the language models fetched with what each costs in memory and what this machine can hold `[x]`, the same lists in the settings `[x]`. What is left is a pinned checksum for a language model and more than one of them, neither of which can be had from a cloud session, because huggingface.co is not reachable from one | `[~]` |
